...
def get_current_position():
    ...
    try:
        ...
        logger.info(f"Current position: {qty} shares of {SYMBOL} | Avg buy price: ${avg_price:.2f} | Current: ${current_price:.2f}")
        return position
    except Exception:
        # No position found — we don't own SPY right now
        logger.info("No current position in SPY")
        return None
...
def buy_spy():
    """
    Buy SPY using 10% of our available cash balance.
    ...
    order = MarketOrderRequest(
        ...
        side=OrderSide.BUY,
        time_in_force=TimeInForce.DAY  # Order expires at end of trading day if not filled
    )
...
def sell_spy():
    """
    Sell ALL shares of SPY we currently own.
    Uses a Market Order — sells immediately at the current market price.
    """
    position = get_current_position()

    if position is None:
        logger.info("Tried to sell but we don't own any SPY")
        return None
    ...
    order = MarketOrderRequest(
        symbol=SYMBOL,
        qty=qty,           # Sell all shares we own
        side=OrderSide.SELL,
        time_in_force=TimeInForce.DAY
    )

    result = trading_client.submit_order(order)
    logger.info(f"SELL order placed: {qty} shares of {SYMBOL} | Order ID: {result.id}")
    return result
